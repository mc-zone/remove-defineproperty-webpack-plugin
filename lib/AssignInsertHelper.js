...
module.exports = class AssignInsertHelper {
  ...
  walkAssignmentExpression(expression, ignores){
    ...
    if(left.type === "Identifier" && !ignores.has(left.name) && this.assigns.has(left.name)){
      this.insertAssign(left, left.name);
    }
  ...
  walkUpdateExpression(expression, ignores){
    ...
    if(argument.type === "Identifier" && !ignores.has(argument.name) && this.assigns.has(argument.name)){
      this.appendUpdate(expression, argument.name);
    }
  }
  ...
  _walkFunction(expression, ignores){
    ...
    if(expression.body){
      if(expression.expression === true){
        return this.walkStatement(expression.body, ignores);
      }else{
